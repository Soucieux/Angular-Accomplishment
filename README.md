...
| **Reminder** | Account expense tracker with real-time cloud sync.<br>- **Inline cell editing** with keyboard confirm flow<br>- Colour-coded active, overdue, and paid-off status indicators<br>- **Start / end time slots** — optional HH:MM time range per item with auto-computed end offset<br>- Persistent per-user state | Active |
...
│   ├── locale/                   # Locale strings — locale.en.ts (English, single source of truth)
