...
| **Reminder** | Account expense tracker with real-time cloud sync.<br>- **Inline cell editing** with keyboard confirm flow<br>- Colour-coded active, overdue, and paid-off status indicators<br>- **Start / end time slots** — optional HH:MM time range per item with auto-computed end offset<br>- **Per-card delete button** for quick single-item removal<br>- Persistent per-user state | Active |
