...
The [Vision Canvas Field Guide](visualization-guides/index.html) documents all 13 routed pages in mirrored English and Chinese editions.

### Where to open it

| Platform | Launcher |
| :--- | :--- |
| Desktop · web · PWA · Tauri | Book button in the lower-right corner |
| Mobile · Capacitor | Top of the expanded **All sections** panel |

Every launcher resolves to the canonical production domain, never localhost or a provider-owned address.

### How it ships

| Form | Detail |
| :--- | :--- |
| **From disk** | The responsive source opens directly — no server required |
| **With the app** | Copied to `/guide/` in the Angular production build |
| **On the web** | Published at [visioncanvas.net/guide](https://www.visioncanvas.net/guide/index.html) — no sign-in required |
| **Offline** | Generates one self-contained HTML file for transfer between devices |

### Evidence standard

Every chapter covers the applicable workflows, validation, loading, empty, permission, recovery, responsive, and destructive states, illustrated with **179 privacy-safe screenshots captured from the real running application** rather than generated interface reconstructions.

The 13 routed pages plus a shared **Messages & Errors** reference make 14 chapters in total — all complete as of v2.4.0, with every documented scenario paired to a rendered capture.

Select any screenshot to enlarge it in place; the in-page viewer never opens a separate browser tab. The guide contains no PDFs.
...
| **Entertainment** | Media tracker for **movies and TV dramas** with rich metadata support.<br>- **Third-party API** lookup for title, genre, cast, and cover image, with a **scrape fallback** that backfills a missing rating or first release date<br>- Firebase / Cloudbase image storage and CDN delivery<br>- **Text search** across titles, plus category filtering, favourites toggle, and **colour-coded rating indicators**<br>- Full add / deletion history with timestamps | Active |
| **Reminder** | Account expense tracker with real-time cloud sync.<br>- **Inline cell editing** with keyboard confirm flow<br>- Colour-coded active, overdue, and paid-off status indicators<br>- **Start / end time slots** — optional minute-precise HH:MM range; the end may equal but never precede the start, and is automatically set one hour later with a 23:59 cap<br>- **Per-card delete button** for quick single-item removal<br>- **Shared reminders** *(username / password accounts only — cross-account sharing runs on CloudBase)* — items marked shared appear on connected accounts in real time, with a shared-only filter chip and creator badges<br>- Persistent per-user state | Active |
| **Portal** | Personal AI search hub and link manager.<br>- **Chip-based category filters** for AI tool discovery with category management dialog<br>- Saved link management with category assignment; site titles are auto-fetched with a bounded wait, remain manually editable, and are **auto title-cased**; favicons resolve to **real brand logos** (Brandfetch, with a favicon fallback)<br>- **Date Calculator** — date-range arithmetic with CloudBase persistence; collapsible via header click<br>- Shared link flag to distinguish personal from shared entries<br>- Fallback colour avatars when logo fetch fails | Active |
...
| **Resonance** | Personal quote vault.<br>- Author attribution and timestamps<br>- **Anonymous browsing** support for public quotes<br>- Ownership-based edit and delete permissions | Stable |
...
Separation is by design and applies to shared pools too: a CloudBase user never sees Firebase's shared data, and a Google user never sees CloudBase's. Neither backend mirrors the other, and nothing syncs between them at runtime.

Firebase also fills a **second, unrelated role — backup storage.** A scheduled job (`.github/workflows/backup.yml`, `cron: '0 3 * * *'`, 03:00 UTC daily) copies every CloudBase collection into Firebase, paging at 1000 documents to stay within CloudBase's per-query cap. Those copies land under a dedicated `backup` root and are **never read by the app**. That root exists precisely so a backup run can never overwrite the live nodes serving Google-sign-in users. Backup data and live Firebase data are two separate stores that happen to share a project.
...
| URL proxy (Portal RSS / titles) | `fetchUrl` Cloud Function | `proxyFetch` callable Cloud Function |
