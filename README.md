...
| **Vault** | Account links graph with a **Slate Steel** theme.<br>- **Force-directed graph** of accounts (app-tile nodes, coloured by category — **segmented when an account holds several**), emails (circles), phones (diamonds), and links (hexagons)<br>- **Click-to-trace** — highlights linked nodes by hop distance (direct / second-degree) and dims the rest; drag, pan, and zoom<br>- **Category overview** chips with **inline rename / delete**; the **legend doubles as a type filter** — click a type (or Verified) to isolate it, click again to clear<br>- **Preset + custom categories** — five built-in presets (Transport · Finance · Social · Shopping · General), each with a unique colour and icon, plus your own<br>- **Add-account dialog** with **multi-select categories**, a duplicate-name guard, and multi-connection wiring; **link-mode** to connect nodes by hand<br>- **Auto-fit map** opens framed to every node and keeps them inside the view, stepping the base scale down as the graph grows; **inline account-name editing**<br>- List view with inline link editing, **multi-category assignment**, and **free-form note connections** kept off the graph; on narrow screens the graph gives way to a **List view** prompt; per-user CloudBase persistence | Beta |
