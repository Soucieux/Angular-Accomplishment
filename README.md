...
| **May 2026** | Vision Home orbital dashboard launched — Life Clock with real-time clock display and concentric year / month / week / day progress rings; satellite stat discs; quick-action pill buttons; mobile-responsive layout. Login page entrance animations added. Sign-up flow completed — new users can register directly from the login page. |
...
| **June 2026** | Three-mode navigation introduced: desktop (≥1300px) retains the collapsible side drawer; a new compact overlay mode (941–1300px) adds a floating hamburger button that opens the drawer as a full-width overlay with a dark backdrop; mobile (≤940px) continues using the bottom navigation bar. Account actions (avatar, online status, sign-out) added to the mobile bottom nav. Vision Home orbital dashboard expanded — additional widget panels added including week calendar, activity feed, debt tracker, recipe list, and quick-access link chips; concentric ring sizing made responsive for narrow viewports. PWA support added — app is installable as a home screen icon on iOS and Android with a web manifest, themed status bar, and safe-area insets for notched devices. Web push notifications added — users on real mobile devices can subscribe via the account popover in the bottom nav; a daily digest notification fires at 09:00 CST via a CloudBase scheduled function that checks all active subscriptions and pushes for any reminder due that day. Subscription state is stored per-user in the database and cleared on sign-out. |
