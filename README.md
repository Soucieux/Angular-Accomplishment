...
| **August 2026** | - The loading-timeout **retry dialog now closes itself** when the awaited data finally arrives, so a slow-but-healthy load no longer leaves a modal covering a page that is already usable; a retry click landing in that same moment is ignored instead of discarding the finished load<br>- A retry dialog raised by an **expired session** is left untouched by that auto-close, since both share the same dialog type<br>- **Home** — the shortcuts panel's *see all* row now appears whenever the list is capped, not only when it holds exactly 20 items; its Chinese label wording corrected<br>- **User Guide** — four more real-app captures added, covering the populated Entertainment, Recipe, and Debt panels on Home plus the Patch Notes activity heatmap |
