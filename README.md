...
The web-first [Vision Canvas Field Guide](visualization-guides/index.html) documents all 13 routed pages in mirrored English and Chinese editions. The same responsive source opens directly from disk, is copied to `/guide/` in the Angular production build, is publicly available at `https://www.visioncanvas.net/guide/index.html` without requiring sign-in, and generates one self-contained offline HTML file for transfer between devices. Open it from the small book button in the lower-right corner on desktop, web, PWA, and Tauri, or from the top of the expanded **All sections** panel on mobile and Capacitor; every launcher uses the canonical production domain rather than localhost or a provider-owned domain. The guide no longer includes PDFs. Select any instructional screenshot to enlarge it on the current screen; the in-page viewer never opens a separate browser tab. Reminder establishes the guide-wide evidence standard: every chapter explains applicable workflows, validation, loading, empty, permission, recovery, responsive, and destructive states with privacy-safe screenshots captured from the real running application rather than generated interface reconstructions.
